...
export default function QuizNormal() {
  ...
  useEffect(() => {
    if (btn_click === 10) {
      document.getElementById("QuizNormalResult_btn").style.display =
        "inline-block";
    }
  }, [btn_click === 10]);
  ...
  return (
    <>
      ...
      {c.QuizNormal.length > 0 ? (
        c.QuizNormal.map((item, index) => (
          <div key={item.id}>
            <h3 className="text-left" align="left">
            ...
            <div>
              <button
                id={item.id}
                className={`btn btn-outline-secondary submitbtn ${item.id}`}
                onClick={() => {
              ...
              >
                <i class="bi bi-check2-circle p-0 m-0"></i>
              </button>
            </div>
          </div>
        ))
      ...
      <div className="text-center">{checkE && <QuizHard />}</div>
    </>
  );
}
